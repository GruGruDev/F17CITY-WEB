const app = {
  ...
  dateDiff: function (date1, date2 = new Date()) {
    ...
    if (secondsDiff < 60) {
      ...
      return { unit: unit, value: secondsDiff };
    } else if (minutesDiff < 60) {
      unit = minutesDiff === 1 ? "minute" : "minutes";
      return { unit: unit, value: minutesDiff };
    } else if (hoursDiff < 24) {
      unit = hoursDiff === 1 ? "hour" : "hours";
      return { unit: unit, value: hoursDiff };
    } else if (daysDiff < 7) {
      unit = daysDiff === 1 ? "day" : "days";
      return { unit: unit, value: daysDiff };
    } else if (weeksDiff < 4) {
      unit = weeksDiff === 1 ? "week" : "weeks";
      return { unit: unit, value: weeksDiff };
    } else if (monthsDiff < 12) {
      unit = monthsDiff === 1 ? "month" : "months";
      return { unit: unit, value: monthsDiff };
    } else {
      unit = yearsDiff === 1 ? "year" : "years";
      return { unit: unit, value: yearsDiff };
    }
  },
